...
#
# В filter можно передавать не только самописные функции но и уже имеющиеся, но важно чтобы функция возвращала либо
# True либо False – одна из таких является функция bool() которая возвращает Тру если число не является пустым
# множеством(0)
#
...
#
# b = list(filter(lambda x: x % 3 == 0, a)) – функция lambda в которую аргументом x поступают по очереди элементы
# списка а и если элемент делиться на 3 без остатка, возвращается элемент.
#
...
# a = '13opj13i4h[1o'
#
# b = list(filter(str.isdigit, a)) – передаем аргументом строку которая проверяется методом isdigit() – тоесть
# isdigit возвращает тру если элемент является числом.
#
